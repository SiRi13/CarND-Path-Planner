...
void BehaviourPlanner::_lane_change_center() {
  ...
  if (cars_ahead_center.size() > 0) {
    ...
    /* printf("car[%i] ahead @ %f in %f\n", (int)cars_ahead_center[0][0],
           ...
           */
    if (cars_ahead_center[0][1] <= MIN_SAFETY_DIST) {
      ...
      if (cars_ahead_left.size() > 0) {
        std::sort(
            cars_ahead_left.begin(), cars_ahead_left.end(),
            [](vector<double> a, vector<double> b) { return a[1] < b[1]; });
      }
      if (cars_ahead_right.size() > 0) {
        std::sort(
            cars_ahead_right.begin(), cars_ahead_right.end(),
            [](vector<double> a, vector<double> b) { return a[1] < b[1]; });
      }
      if (cars_behind_left.size()) {
        std::sort(
            cars_behind_left.begin(), cars_behind_left.end(),
            [](vector<double> a, vector<double> b) { return a[1] < b[1]; });
      }
      if (cars_behind_right.size()) {
        std::sort(
            cars_behind_right.begin(), cars_behind_right.end(),
            [](vector<double> a, vector<double> b) { return a[1] < b[1]; });
      }
...
void BehaviourPlanner::_lane_change_right() {
  ...
  vector<vector<double>> cars_ahead_right = _peerCarsAhead[RIGHT_LANE];
  // printf("number of cars ahead right: %i\n", (int)cars_ahead_right.size());
  if (cars_ahead_right.size() > 0) {
